...
def _dot(x, y):
  return tf.reduce_sum(input_tensor=x * y, axis=-1)


def _dot_cossim(x, y, cossim_pow=0):
  eps = 1e-4
  xy_dot = _dot(x, y)
  if cossim_pow == 0: return tf.reduce_mean(input_tensor=xy_dot)
  x_mags = tf.sqrt(_dot(x,x))
  y_mags = tf.sqrt(_dot(y,y))
  cossims = xy_dot / (eps + x_mags ) / (eps + y_mags)
  floored_cossims = tf.maximum(0.1, cossims)
  return tf.reduce_mean(input_tensor=xy_dot * floored_cossims**cossim_pow)


def _extract_act_pos(acts, x=None, y=None):
  shape = tf.shape(input=acts)
...
def _T_force_NHWC(T):
  ...
  def T2(name):
    ...
    if len(shape) == 2:
      ...
      if fmt == "NCHW":
        return tf.transpose(a=t, perm=[0, 2, 3, 1])
    return t
  return T2
